...
def Devercode():
    try:
        ...
        VercodeToken = requests.get(GetVercodeUrl, headers=Headers).json()[
            "data"]["Token"]
        ...
        Vercode = pytesseract.image_to_string(
            VercodeImage, config="--psm 7 vercode")  # 命令解释：指定页面分段模式 将图片视作单行文本
        if Vercode != "":
...
def Login(Username, Password):
    try:
        ...
        if Content["msg"] == "验证码错误":
            ...
            CookiesDict = requests.utils.dict_from_cookiejar(
...
def GetInformation(LoginedHeaders):
    try:
        ...
        PersonalDict = requests.get(
            PersonalInfomationUrl, headers=LoginedHeaders).json()
        Name = PersonalDict["data"]["Name"]
        ClassName = PersonalDict["data"]["ClassName"]
        Status = PersonalDict["msg"]
        BaseInfomationUrl = "https://fangkong.hnu.edu.cn/api/v1/clockincount/getcountlist"
        Sum = str(requests.get(BaseInfomationUrl,
                  headers=LoginedHeaders).json()["data"]["AllCount"])
        Information = {
            "全校已打卡人数": Sum,
            "姓名": Name,
            "班级": ClassName,
...
def ClockIn(LoginedHeaders, ClockinInformation):
    try:
        ...
        ClockInDict = requests.post(
            ClockInUrl, headers=LoginedHeaders, data=Data).json()
...
def ClockLine(Account):
    ...
    if (Status == "账号或密码错误"):
        ...
        if (Status[0] == "成功"):
            CInformation = "[打卡信息]\n地区:{}\n地址:{}\n经度:{} E\n纬度:{} N\n早晚体温:{}℃".format(
                District, ClockinInformation[3], ClockinInformation[4], ClockinInformation[5], ClockinInformation[6])
            Message = "[打卡成功] [{}]\n[姓名]{} [班级]{}\n[全校已打卡人数]{}\n{}".format(
